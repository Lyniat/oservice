class Radio
  attr_accessor :enabled, :callback, :arguments, :list

  def initialize(x, y, w, h, list, enabled, *callback_and_args)
    @x = x
    @y = y
    @w = w
    @h = h
    ...
    @list = list
    # these look a bit weird scaled in default font
    @char_disabled = "◎"
    @char_enabled = "◉"
    @enabled = enabled

    if @enabled
      disable_list_elements
    end
